...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("controls", "temporary_speed", 42, 4211, 42),
        ...
        ("comfort", "temporary_temperature", 10, 4213, 20),
...
async def test_invalid_mode(unit: MockModbusUnit, value: object) -> None:
    with pytest.raises(ValueError):
        await AirPack4(unit).controls.write("operating_mode", value)
...
async def test_boolean_write_is_strict(unit: MockModbusUnit, value: object) -> None:
    with pytest.raises(ValueError):
        await AirPack4(unit).controls.write("enabled", value)
...
async def test_invalid_temperature(unit: MockModbusUnit, value: object) -> None:
    device = AirPack4(unit, options=DeviceOptions(comfort=True))
